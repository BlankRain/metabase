...
(defrecord ^{DisallowConcurrentExecution true} SyncAndAnalyzeDatabase []
  org.quartz.Job
...
(defrecord ^{DisallowConcurrentExecution true} UpdateFieldValues []
  org.quartz.Job
  (execute [_ job-context]
    (let [database (job-context->database job-context)]
      (when (:is_full_sync database)
  ...
  (triggers/build
   ...
   (triggers/with-schedule
     (cron/schedule
      ...
      ;; row (i.e. more than an hour goes by) then execute still, but drop the additional occurrences of the same
      ;; trigger
